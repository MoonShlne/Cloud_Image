...
把上传的图片通过腾讯云的 数据万象，添加新规则，在每一次上传的时候，会为每个规则上传一份图片，
...
懒加载 ，前端优化，先获取到图片，视图看到什么内容，加载什么内容

渐进式加载   渐进式加载和懒加载技术类似,先加载低分辨率或低质量的占位资源(如模糊的图片缩略图),在用户访问或等待期间逐步加载高分辨率的完整资源,加载完成后再替换掉占位资源。
适用于超清图片加载、用户体验要求较高的页面,在网络环境较差时,效果会更明显。   （前端组件库实现）
...
session持久化，使用redis session  只用修改ymal 和加入redis session 依赖，存入redis需要实现Serializable类 

1. 为什么要实现 Serializable
Spring Session 默认使用 JDK 序列化（DefaultSerializer），存入 Redis 的对象必须实现 Serializable。
如果对象未实现 Serializable，会报错：DefaultSerializer requires a Serializable payload...。
2. Jackson 与 JDK 序列化的区别
Jackson 主要用于 HTTP 请求/响应的 JSON 序列化，不自动用于 Session 或 Redis 的对象存储。
Spring Session 默认不是用 Jackson，而是用 JDK 序列化。
