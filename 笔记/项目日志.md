...
对于调用ai模型，  后端调用ai，ai计算， 如果一直跟ai连接，那么非常影响后端的性能



所以请求后端后先返回一个id，然后放到队列，
...
根据任务id 查询任务进度  如果succeed  则获得 img url 返回
...
由于该功能又300次免费额度，所以上线要进行限制，防止用户刷完

ai扩图功能，  对于图片太大或者太小 会导致任务失败 由于为了节省云储存空间，上传的图片都进行过压缩，所以图片都质量偏低容易导致扩图失败，具体信息看请求的返回值信息

![image-20251026184656314](D:\porjects\Cloud_Image\cloud-image\笔记\img\limit.png)

以及使用非大陆`vpn`会导致 `errorcode` **Invalid API-key provided.**
...
对于查询数据，比如只要查询size 查询条件就只用size 不用 * 来优化sql

返回查询结果时候，不返回 list<pojo> 而是 返回 list<string>  来节约内存 
...
//优化sql
queryWrapper.select("category", "COUNT(*) AS count", "SUM(pic_size) AS totalSize").groupBy("category");
//查询
...
在空间增加图片， space表的 total Size 的大小会变大，每次插入图片都会变大，但是修改图片是把修改的图片大小加进去，导致size跟 picture表的count 不一致
...
然后根据构造的字段进行groupby  就可以得到时间粒度区间的信息
...
![Untitled diagram-2025-10-31-054643](D:\porjects\Cloud_Image\cloud-image\笔记\img\Untitled diagram-2025-10-31-054643.png)



**AOP 拦截 (入口)**：

- 客户端请求您 Controller 中带有 `@SaSpaceCheckPermission` 注解的方法。
- 您自定义的 AOP 切面（`SpacePermissionAspect`）拦截到该注解。

**切面处理 (AOP)**：

- AOP 从注解中解析出**权限码**（如 `"picture:upload"`）和**登录体系类型**（`type = "space"`）。
- AOP 调用 Sa-Token 核心 API，即 `StpKit.SPACE.checkPermission(权限码)`。

**Sa-Token 核心鉴权 (StpLogic)**：

- **(a) 身份识别 (`getLoginId`)**：`StpKit.SPACE` 实例首先执行 `getTokenValue()` 从请求（Header 或 Cookie）中获取 Token。
- **(b) Token 换 ID**：通过 `getLoginIdByToken(token)`，从 Redis 缓存中反查出 `loginId`。
  - *失败*：如果 Token 不存在或无效，`getLoginId()` 会直接抛出 `NotLoginException`（未登录）异常，流程终止。
- **(c) 权限加载 (`getPermissionList`)**：
  - Sa-Token 回调您实现的 `StpInterface.getPermissionList(loginId, "space")` 方法。
  - 您的方法根据 `loginId` 和业务上下文（如 `spaceId`）查询数据库，返回该用户在 `space` 体系下拥有的**所有权限**（一个 `List<String>`）。
- **(d) 权限比对**：Sa-Token 核心将“AOP 要求”的权限码与“`getPermissionList` 返回”的权限列表进行比对。

**放行或拦截 (出口)**：

- **比对成功**：`checkPermission` 方法正常返回。AOP 切面执行 `pjp.proceed()`，请求**放行**，进入 Controller 业务方法。
- **比对失败**：`checkPermission` 抛出 `NotPermissionException`（无权限）异常。
- **全局异常**：所有 Sa-Token 抛出的异常（`NotLoginException` / `NotPermissionException`）被您的全局异常处理器捕获，并向客户端返回 401 或 403 错误。



![Untitled diagram-2025-11-01-081454](D:\porjects\Cloud_Image\cloud-image\笔记\img\Untitled diagram-2025-11-01-081454.png)



以及在返回前端数据时，返回用户的权限，让前端知道用户有哪些权限，可以不展示没权限的按钮
...
1.后端服务重启后，Sa-Token 的 token 与 loginId 的关系没有持久化到 Redis，而是存储在内存（默认 SaTokenDao）。重启后内存数据丢失，前端虽然还带着 token，但后端无法通过 token 获取到 loginId，导致权限校验失败。只有前端重新登录，后端才会重新建立 token 与 loginId 的映射，权限校验才能恢复正常。  简要总结： 没有将 Sa-Token 的会话数据（token 与 loginId 的关系）持久化到 Redis，导致服务重启后内存丢失，token 失效，权限校验失败。

2.导包不一致不知道是新特性还是版本问题导致了自定义注解校验失效

3.在校验权限时，业务逻辑里如果图片找不到就返回无图片信息，而不是不需要权限校验

4.在测试时，“浏览器 Cookie 共享” ，在同一个域下 例如localhost:5137 ，浏览器会共享同一个域，导致测试权限有问题，其实是账号不一致
...
本项目使用 Apache ShardingSphere 来实现分库分表。



1. ### 静态分表 (Static Sharding)

- **定义**： 在设计阶段，分表的数量和规则就是**固定**的，不会根据业务增长动态调整。
- **分片规则**： 通常基于某一字段（如图片 ID），通过简单规则（如取模、范围）来决定数据存储在哪个表或库中。
- **示例**：
  - `picture_0`
  - `picture_1`
- **优缺点**：
  - **优点**：实现简单，易于理解。
  - **缺点**：扩展性差。随着数据量增长，可能需要手动调整分表数量并迁移数据。

2. ### 动态分表 (Dynamic Sharding)

- **定义**： 分表的数量可以根据业务需求或数据量**动态增加**，表的结构和规则是在运行时动态生成的。
- **示例**：
  - 根据时间动态创建 `picture_2025_01`、`picture_2025_02`。

### 3. 本项目应用：动态分表

- **实现方式**： 本项目采用动态分表策略（这在 ShardingSphere 中也常被称为“租户分片”）。
- **业务目标**： 为 **VIP 用户（旗舰版空间）** 提升查询效率，实现数据隔离。
- **具体规则**： 为**每一个**旗舰版的空间**单独创建一张表**。
  ...
  shardingsphere:
    datasource:
      ...
      cloud_image:
        ...
        url: jdbc:mysql://localhost:3306/cloud_image
        username: root
        password: 123456
      rules:
        sharding:
...
`application.yml` 的 `spring.shardingsphere` 部分 定义数据源、分片规则和分片算法引用。核心点：
逻辑表是 picture。
分片键是 `spaceId`。
引用自定义算法 `picture_sharding_algorithm`，让请求携带的 spaceId 决定路由到 `picture_\{spaceId}` 或回退到逻辑表 picture。
PictureShardingAlgorithm 决定“精确查询带 spaceId 时”路由到哪张物理表：
有 `spaceId` 且存在对应表，就路由到 `picture_\{spaceId}`。
没有对应表或 `spaceId` 为空，就落到逻辑表 picture。
`DynamicShardingManager` 运行期动态管理分片：
启动时扫描需要分表的空间，组装 `actual-data-nodes` 并下发到 `ShardingSphere`（热更新，无需重启）。
新建“旗舰版团队空间”时自动 CREATE TABLE picture_\{spaceId} LIKE picture，并刷新路由，使新表立刻可用。

<img src="C:\下载\Untitled diagram-2025-11-01-141203.png" alt="Untitled diagram-2025-11-01-141203" style="zoom:33%;" />



<img src="C:\下载\Untitled diagram-2025-11-01-142026.png" alt="Untitled diagram-2025-11-01-142026" style="zoom:33%;" />



### `ShardingSphere` 分表逻辑总结



1. **核心策略：按 `spaceId` 动态分表 (租户隔离)**
   - 系统以 `spaceId` 作为**分片键（Sharding Key）**。
   - 采用动态分表策略，为特定的（例如旗舰版）`space` 自动创建一张**专属的物理表**（例如 `picture_{spaceId}`）。
2. **查询性能：`spaceId` 是路由的关键**
   - **高效查询（精确分片）**：
     - 当 `SELECT` 查询的 `WHERE` 条件中**指定了 `spaceId`** 时（例如 `... WHERE spaceId = 101`）。
     - ShardingSphere 的自定义分片算法会启动，将查询**精确路由**到**唯一**的目标物理表（`picture_101`）。
     - **结果**：查询性能极高，实现了 VIP 用户的查询效率提升。
   - **低效查询（广播查询）**：
     - 当 `SELECT` 查询**未指定 `spaceId`** 时。
     - ShardingSphere 无法确定目标表，因此必须执行**广播查询（Broadcast Query）**。
     - **结果**：框架会将该查询同时发送到**所有**已知的物理分表（包括 `picture`、`picture_101`、`picture_102`...）上执行，然后合并所有结果。这会导致**性能极低**，应在业务中严格避免。
3. **强制规则：写操作必须携带分片键**
   - 对于所有的**数据写入/修改**操作（`INSERT`、`UPDATE`、`DELETE`），**必须在 SQL 中指定 `spaceId`**。
   - **原因**：如果不提供 `spaceId`，ShardingSphere 框架**无法识别**这行数据应该被插入（或修改）到哪一张具体的物理表中，这将导致操作**路由失败**。
...
![Untitled diagram-2025-11-02-070312](D:\porjects\Cloud_Image\cloud-image\笔记\img\Untitled diagram-2025-11-02-070312.png)
...
![Untitled diagram-2025-11-02-073336](D:\porjects\Cloud_Image\cloud-image\笔记\img\Untitled diagram-2025-11-02-073336.png)
...
所以使用 前端发来请求， 后端创建线程来处理任务  使用disruptor

其实上述的异步操作+从任务队列取任务执行，使用线程池就可以实现了。
但对于协同编辑场景,需要尽可能地保证低延迟,因此我们选用一种高级技术Disruptor无锁队列来减少线程上下文的切
换,能够在高并发场景下保持低延迟和高吞吐量。
此外,使用Disruptor还有一个优点,可以将任务放到队列中,通过优雅停机机制,在服务停止前执行完所有的任务,再
退出服务，防止消息丢失。

它主要通过以下几点实现高性能的消息传递机制：
1,环形缓冲区:使用固定大小的数组,可以复用内存,避免了频繁的内存分配和垃圾回收。
2·无锁设计:依赖CAS (Compare-And-Swap)和内存屏障,而不是传统的锁,降低了线程切换的开销。
3,缓存友好:最大化利用CPU的缓存局部性,提高访问速度。
...
![Untitled diagram-2025-11-02-114625](D:\porjects\Cloud_Image\cloud-image\笔记\img\Untitled diagram-2025-11-02-114625.png)

# bug

如果我管理员删除了一个空间，那么空间里的用户信息还在，每次用户调用获取自己空间的方法都会获得，但是进不去，而且访问不到了，所以应该在删除数据库时删除用户信息
